...
{
    [TestClass]
    public ref class E04_ProblemsSpec
    {
    private:
    ...
    public:
        ...
        void NWarSample1_TestCase()
        {
            ...
            int *ans = getSquareLeaders(arr, len, &ansLen);
        ...
        {
            int n = 0,i=0;
            int result = getSteps(0);
            int inputs[6] = { 3, 5, 7, 9, 10, 12 };
            int answers[6] = { 3, 8, 21, 55, 89, 233 };
            for (i = 0; i < 6; i++){
				Assert::AreEqual(answers[i], getSteps(inputs[i]), L"Failed for either 3,5,7,9,10,12", 1, 2);
            }
        }
    };
}
